...
- Явные диагностические ошибки при парсинге элементов массива (с индексом и путём файла).
- Опциональная запись в «красивом» виде (`JsonEncoder.withIndent('  ')`).
- Гарантировать существование каталога перед записью (`await File(path).parent.create(recursive: true)`).
- Интерфейс репозитория для подмены хранилища (файл/БД/HTTP).
