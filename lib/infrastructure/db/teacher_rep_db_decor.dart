...
class TeacherRepDbDecor extends TeacherRepBase{
  ...
  Future<List<TeacherInfo>> getKthNShortList({required int k, required int n}) async {
    ...
    final orderSql = sort == null ? 'ORDER BY lower(last_name), lower(first_name), id' : 'ORDER BY ${sort!.toOrderBySql()}';
  ...
  Future<int> getCount() async {
    final whereSql = filter.where.trim().isEmpty ? '' : 'WHERE ${filter.where}';
    return db.scalarInt('SELECT COUNT(*)::int FROM teachers $whereSql', params: filter.params);
  }
}